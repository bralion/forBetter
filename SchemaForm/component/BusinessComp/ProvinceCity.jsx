...
const ProvinceCity = (props) => {
    const { value: { prov, city } } = props
    const { onChange, onFormItemChange } = props
    const [cities, setCities] = useState(cityData[prov])

    const handleProvinceChange = value => {
        if (onChange) {
            onChange({ prov: value, city: '' })
        }
        if (onFormItemChange) {
            const { kId = '', title = '' } = props
            onFormItemChange({ kId, value: { [title]: { prov: value, city: '' } } })
        }
    ...
    const onSecondCityChange = value => {
        if (onChange) {
            onChange({ prov: prov, city: value })
        }
        if (onFormItemChange) {
            const { kId = '', title = '' } = props
            onFormItemChange({ kId, value: { [title]: { prov: prov, city: value } } })
        }
    };
    return (
        <>
            <Select value={prov} style={{ width: 120, marginRight: 20 }} onChange={handleProvinceChange} placeholder="请选择省份" >
                {provinceData.map((province, index) => (
                    <Option value={province} key={index}>{province}</Option>
                ))}
            </Select>
            <Select style={{ width: 120 }} value={city} onChange={onSecondCityChange} placeholder="请选择城市">
                {
...
export default forwardRef(ProvinceCity)
