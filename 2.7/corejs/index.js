...
class dashboard {
  ...
  /**
   * 通知顶层窗口
   * @param {String} code 通信码
   * @param {String} type 类型
   * @param {Any} payload 携带物
   */
  noticeTopWindow(code, type, payload) {
    if (!code && window.dashboardMode) {
      code = window.dashboardMode.tabId;
    }
    ...
    if (window.top && window.top.ievent) {
      window.top.ievent.trigger(code, { type, payload });
    }
  }
}
