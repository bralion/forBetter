...
export default class QueryManager {
    ...
    doQueryBySql(cfg, callback) {
        let { query, jndi } = cfg.chartDefinition.query;
        //默认sql查询
        if (!query || !jndi) {
            callback({});
        }
     ...
     */
    doQueryByDataset(cfg, datasetSetting, callback, drillData) {
        let _dataset = new dataset();
        let { query, option } = cfg.chartDefinition;
        let isDrill = option.clickEvent === "drill";
     ...
     * @param {Object} drillData 下钻数据[数据集]
     */
    doQuery(cfg, callback, drillData) {
