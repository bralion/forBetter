...
import "dashboard_2_7/corejs/resource/style/font-awesome.css";
import canvg from "dashboard_2_7/corejs/resource/js/canvg";
import { message } from "antd";
import util from "dashboard_2_7/corejs/components/table/js/util";
...
const domtoimage = require("@/lib/domtoimage");
...
class Util {
    ...
    downToPdf(node, name){
        ...
        this.downToPng(node, name, function(blob){
            readBlobAsDataURL(blob, function (dataurl){
                let canWidth = node.clientWidth;
                let canHeight =  node.clientHeight;
                let t = 'p';
                if(canWidth > canHeight){
                    t = "l";
                    canWidth = node.clientHeight;
                    canHeight =  node.clientWidth;
                }
                let pdf = new jsPDF(t, 'px',[canWidth,canHeight],true);
    ...
    downToPng(node, name, resolve) {
        ...
        if (window.navigator.msSaveOrOpenBlob) {
            ...
            if (typeof html2canvas !== "undefined") {
                ...
                let nodesToRecover = [];
                let nodesToRemove = [];
                let svgElem = [...node.querySelectorAll("svg")]; //divReport为需要截取成图片的dom的id
            ...
            html2canvas(shareContent, opts).then(function(canvas) {
                ...
                let blob = _this.dataURLtoBlob(url);
                if (resolve) {
                    resolve(blob);
                } else {
    ...
    popTips(type, info) {
        ...
        if (popTips) {
            ...
            let myTips = document.createElement("div");
    ...
    lnglatVerify(lng, lat) {
        let check = true;
        let lngFormat = /^(\-|\+)?(((\d|[1-9]\d|1[0-7]\d|0{1,3})\.\d*[1-9])|(\d|[1-9]\d|1[0-7]\d|0{1,3})|180\.0+|180)$/;
        let latFormat = /^(\-|\+)?([0-8]?\d{1}\.\d*[1-9]|90\.0+|[0-8]?\d{1}|90)$/;
