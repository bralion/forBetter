...
export default class WidgetBase extends Observe {
    ...
    init() {
        //缓存下转
        if (this.cfg.drillData) {
            this.drillData = { ...this.cfg.drillData };
            this.cfg.drillData = null;
        }
        ...
        //sql初始化抑制查询
        let { query, jndi, type } = this.cfg.chartDefinition.query || {};
        if (type === "sql" && (!query || !jndi)) {
        } else {
            this.doQuery("", "", this.drillData);
        }
        [
            /* "doQuery", */ "widgetWillCreated",
            "widgetDidCreated",
            "draw"
        ].forEach(fun => this[fun].call(this, this.cfg));
    }
     ...
     * @param {Object} drillData 下钻数据[数据集]
     * @return {Promise}
     */
    doQuery(callback, initor, drillData) {
        /*  */
        //数据源存在
        ...
        return new Promise((resolve, reject) => {
            window.Dashboard.queryManager.doQuery(
                this.cfg,
                data => {
                    this.cfg.chartDefinition.query.isQueryed = true; //是否查询完成字段

                    if (data.success === false) {
                        if (drillData && queryType === "dataset" && !this.cfg.chartDefinition.defaultDrillDataIsNotNull) {
                            //数据集下钻时无数据不展示
                            message.warn("该成员下级无数据");
                            this.doQuery();
                            resolve({});
                            return false;
                        }
