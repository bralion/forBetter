import WidgetBase from "dashboard_2_7/corejs/components/base";
import Immutable, { Map, is } from "immutable";
import _ from "lodash";
import Util from "dashboard_2_7/corejs/util/util";
...
export default class handle extends WidgetBase {
        ...
        switch (item) {
            ...
            case "baseLine":
                ...
                if (/^\d+$/.test(ownOption[item] + "")) {
                    ...
                    let s1 = (ownOption[item] + "").split(".")[0];
                    let s2 = (ownOption[item] + "").split(".")[1];
    ...
    _handlewarn(warn, option, data, orientation, stack) {
        ...
        warn.value.forEach(iitem =>{
            let ind = warn.seriesList.findIndex(e=>e === iitem.series);
            let e = option.series[ind + 1];
            ~ind && e && e.data.forEach((item, i) => {
                let _item = selfOption.series[ind].data[i];
                if (
                    new Function(
                        "",
                        "return " + (typeof item === "number" ? item : item.value) + iitem.filter + iitem.value
                    )()
                ) {
                    /* item.itemStyle = item.itemStyle || { normal: {} };
                    item.itemStyle.normal.color = iitem.color; */
                    if (iitem.markType) {
                        ...
                        e.markPoint.itemStyle.color = iitem.color;
    ...
    getDatasetData(_data, ownData, cellToInd, ownOption, drillName) {
        let dataHandle  = {};
        try {
            let counterField = this.cfg.chartDefinition.query.datasetSetting.cellsConfig.yAxis.fields[0];
            dataHandle = counterField.config || {};
            ...
            label: {
                normal: {
                    ...
                    formatter: function(e){
                        let value = e.data.value || e.data;
                        return Util.formatData2Str(value, dataHandle) 
                    }
                }
            },
            markLine: {},
    ...
    _handleData(drillName) {
        ...
        this.setChartData(
            option,
            _arr.map(e => e[0]),
            _arr[0][1].map((e, i) => _arr.map(item => item[1][i])),
            lengend
        );
        return option;
    }
    getChangeDatabb(resultset, lengendArr, ownOption, drillName) {
        var array = [];
        var type,
            selfResultset = {
                // nameAndResult: [], //数据和字段
        ...
        //获取长度
        this._drilName = drillName;
        getIfNull && (this._drilName = false);
        if (getIfNull || min === 1) {
            ...
            resultset.forEach(e => {
                let _name = e
                    ...
                    .join("~")
                    .replace(/(~$)|(~~.*)/g, "");
                array.push([_name, e.slice(min, max + 1)]);
            });
        } else {
            //获取当前数据拼接和值 selfResultset 舍弃垃圾数据
            resultset.forEach(e => {
                let _name = e
                    .slice(0, min)
                    .join("~")
                    .replace(/(~+$)/g, "");
                if (!/(^~)|(~~+)/g.test(_name)) {
                    let len = _name.split("~").length;
                    selfResultset.dataArr[len] =
                        selfResultset.dataArr[len] || [];
                    selfResultset.dataArr[len].push([
                        _name,
                        e.slice(min, max + 1)
                    ]);
                }
            ...
            //遍历添加数据
            for (let selfI = min - 1; selfI > 0; selfI--) {
                selfResultset.dataArr[selfI] =
                    selfResultset.dataArr[selfI] || [];
                let arr = [],
                    nameArr = [],
                    nameObj = {},
                    preArr = selfResultset.dataArr[selfI].map(e => e[1]); //缓存数据
                selfResultset.dataArr[selfI + 1].forEach(e => {
                    let _name = e[0].replace(/~[^~]+$/, "");
                    let data = e[1];
                    if (!preArr.includes(_name)) {
                        if (!nameArr.includes(_name)) {
                            nameArr.push(_name);
                            nameObj[_name] = data;
                        } else {
                            nameObj[_name] = [nameObj[_name], data].reduce(
                                (a, b) => a.map((e, i) => e + b[i])
                            );
                        }
                    }
                ...
                //合并数组
                nameArr.map(e => {
                    arr.push([e, nameObj[e]]);
                });
                selfResultset.dataArr[selfI] = selfResultset.dataArr[
                    selfI
                ].concat(arr);
            }
            //下钻层级ind 和nextInd 需要展示的层级
            if (!drillName) {
                selfResultset.ind = 1;
            } else {
                selfResultset.ind = drillName.split("~").length + 1;
            }
            selfResultset.nextInd = selfResultset.ind + 1;
            if (selfResultset.ind > min) {
                selfResultset.ind = 1;
                selfResultset.nextInd = selfResultset.ind + 1;
            }
            array = selfResultset.dataArr[selfResultset.ind];
            if (selfResultset.ind !== 1) {
                array = array.filter(e =>
                    new RegExp("^" + drillName).test(e[0])
                );
                if (!array.length) {
                    array = selfResultset.dataArr[1];
                }
            }
        }
        return array;
