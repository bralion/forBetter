...
import colTypeCfg from "dashboard_2_7/dashboard/modules/common/colTypeCfg";
        ...
        case "UPDATE_CFG":
            {
                ...
                if (mode === "dataset") {
                    query.type = "dataset";
                    //判断是否是第一次进来  赋值cellToInd 和下转数据
                    if (
                        action.isFirst &&
                        chartDefinition.query.datasetSetting
                    ) {
                        cellToInd =
                            chartDefinition.query.datasetSetting.cellToInd ||
                            null;
                        ins.drillData = state.component.drillData
                            ? {
                                  ...state.component.drillData
                              }
                            : null;
                    }
                    query.datasetSetting =
                    ...
                    state.component.chartDefinition.query = {...query};
                } else if (mode === "sql") {
                    query.type = "sql";
                    Object.assign(query, state.sqlCfg);
                }
        ...
        case "SWITCH_COMPONENT_TYPE":
            {
                ...
                if (newType === "table") {
                    ...
                    preCells.map(e => {
                        preCellsConfig[e.valueKey].fields.map(ee => {
                            ...
                            if (ee.config.collect === "NONE") {
                                ...
                                ee.config.editeName &&
                                    (_ee.editeName = ee.config.editeName);
                            } else {
                    ...
                    cells.forEach(e => {
                        ...
                        arr.forEach(ee => {
                            ee.config.isDataGroup = !e.allowFieldTypes;
                        });
                    ...
                    cells.forEach(e => {
                        var _type = e.collectType.selectValue;
                        let arr = splitCells[_type].splice(
                            0,
                            e.max || splitCells[_type].length
                        );
                        arr.forEach(ee => {
                            ee.config.isDataGroup = !e.allowFieldTypes;
                        });
                    ...
                    preCells.map(e => {
                        let cell = state.cells.find(
                            item => item.valueKey === e.valueKey
                        );
                        preCellsConfig[e.valueKey].fields.map(ee => {
                            var _ee = {};
                            if (ee.config.collect === "NONE") {
                            ...
                            _ee.isDataGroup = /* ee.config.isDataGroup */ !cell.allowFieldTypes;
                            _ee.dataGroup = ee.config.dataGroup;
                            splitCells[
                                ee.config.collect === "NONE" ||
                                !ee.config.collect
        ...
        case "UPDATE_COMPONENT":
            {
                let _cells = [],
                    dataGroupObj = {},
                    isSelfSet = action.isSelfSet;
                for (let key in state.cellsConfig) {
                    _cells = _cells.concat(state.cellsConfig[key].fields);
                    let cell = state.cells.find(e => e.valueKey === key);
                    state.cellsConfig[key].fields.forEach(e => {
                        if (
                            e.field.id + "__" + e.group.id === action.id &&
                            !e.config.dataGroup && action.dataGroupObj
                        ) {
                            dataGroupObj.model =
                                action.dataGroupObj.length >= 100
                                    ? "auto"
                                    : "none";
                            isSelfSet || state.component.type && (dataGroupObj.model='none');
                            Object.assign(e.config, {
                                dataGroup: Object.assign(
                                    {},
                                    action.dataGroupObj,
                                    dataGroupObj
                                ),
                                isDataGroup: !cell.allowFieldTypes
                            });
                        }
                    });
                }
        ...
        case "SWITCH_SELECTED_DATASET":
            {
                if (state.dataset.name !== payload.name) {
                    ...
                    state.cellsConfig = { ...state.cellsConfig };
                    state.filterConfig = { ...state.filterConfig };
                    //切换数据集时在component中同步更新
                    let { query } = state.component.chartDefinition;
                    // query.datasetSetting = query.datasetSetting || {};
                    // query.datasetSetting.dataset = {...state.dataset}
                    query.datasetSetting = {
                        cellsConfig: state.cellsConfig,
                        filterConfig: state.filterConfig,
                        limit: state.limit,
        ...
        case "UPDATE_FIELD_FROM_CELL":
            {
                if (action.valueKey !== undefined) {
                    ...
                    if (
                        ...
                        _upffselfselfConfig.isDataGroup = true;
                    }
