 ...
 */
import service,{createSource} from "dashboard_2_7/dashboard/modules/common/Service";
...
import $ from "jquery";
import DATABIND_CALC_FN_LIST from "dashboard_2_7/dashboard/modules/common/config/DATABIND_CALC_FN_LIST";
...
const databind = {
    ...
    */
    ...
    isConvergeField(formulaStr){
        ...
        reg = new RegExp(convergeFns.join('\\s*?\\(|'),'');
        result = reg.test(formulaStr);
        return result;
    },

    /**
     * 如果计算字段只用了一个聚合函数，将该聚合函数名字返回
     * @param {String} formulaStr 公式字符串
     */
    getSingleConvergeFn(formulaStr){
        let reg;
        let result;
        let match;
        if(convergeFns.length === 0){
        ...
        reg = new RegExp(convergeFns.join('\\s*?\\(|'),'g');
        match = formulaStr.match(reg);
    ...
    getDatasetList() {
        ...
        return service({
            ...
            rep.data = rep.data.map(item=>{
                let { fileName,fileGuid,jumpId } = item;
                return {
                    jumpId,
                    id:fileGuid,
                    name:fileName
    ...
    /**
     * 根据数据集查询对应字段信息
     * @param {String} jumpId jumpId
     */
    getFieldsByDataset(jumpId) {
        return service({
     ...
     * @param {String} maxMinTol 是否是查询最大最小值
     */
    getFieldValues(field, dataset, modelId,maxMinTol) {
        ...
        if (field.calc === true) {
            ...
            if(maxMinTol === true){//查询最大最小值字段
                ['MAXIMUM','MINIMUM','COUNT_DISTINCT'].forEach(e=>{
                    params.columns.push({...field,...{selectedAggType:e}});
                });
            }else{
