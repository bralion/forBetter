...
class DataPop extends Component {
    ...
    initState(dataGroup) {
        ...
        let state = {
            ...
            model,
            length,
            restGrop: restGrop || { check: true, groupName: "其他" }
        };
    ...
    //点击添加分组
    addCondition() {
        let { max, min } = this.props.dataGroup;
        let _values = [...this.state.values];
        let values = _values.filter(
                e => !(e.start === "无限制" && e.end === "无限制")
            ),
            lastvalue = values[values.length - 1];
        lastvalue = lastvalue || { start: min, end: max };
        let start = lastvalue.end === "无限制" ? "" : lastvalue.end,
            _end = lastvalue.start === "无限制" ? "" : lastvalue.start,
            end;
        start = start || _end;
        end = start * 1 + 100;
        _values.push({
     ...
     * @param {事件} e 
     * @param {第几个} ind 
     * @param {类型 start end} type 
     */
    setClear(e, ind, type){
        var values = copyReference(this.state.values),
            _values = values[ind];
            if(_values[type]==='无限制' || _values[type + 'zs'] === '无限制'){
                _values[type + 'zs'] = '';
            }
            this.setState({ values });
    }
    ...
    onChangeNum(e, ind, type) {
        ...
        _values[type + "zs"] = value;
        if (
            /^-?((0|[1-9]\d*(\.?\d+)?)|(0\.\d+))$/.test(value) ||
            value === "无限制"
        ) {
            _values[type] = value;
        }
    ...
    onBlurNum(e, ind, type) {
        ...
        _values[type] = value;
        _values[type + "zs"] = value;
        if (_values.start === "无限制" && _values.end === "无限制") {
                                                    ...
                                                    >
                                                        <Col span={20}>
                                                            <Input
                                                                ...
                                                                onFocus={e=>this.setClear(e,i,'start')}
                                                                onChange={e =>
                                                                    this.onChangeNum(
                                                                        e,
                                                                        i,
                                                    ...
                                                    >
                                                        ...
                                                        <Col span={20}>
                                                            <Input
                                                                ...
                                                                onFocus={e=>this.setClear(e,i,'end')}
                                                                onChange={e =>
                                                                    this.onChangeNum(
                                                                        e,
                                                                        i,
                    ...
                    <Button
                        ...
                        onClick={e => {
                            ...
                            if (model === "custom") {
                                ...
                                console.log("restGrop", restGrop);
                                if (
                                    _values.find(e => !e.name) ||
                                    (restGrop.check && !restGrop.groupName)
                                ) {
