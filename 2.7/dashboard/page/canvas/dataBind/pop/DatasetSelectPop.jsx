...
class DatasetSelectPop extends Component {
    ...
    getFieldsByDataset(jumpId) {
        return databindM.getFieldsByDataset(jumpId);
    }
    ...
    switchSelectedDataset(dataset) {
        ...
        //更新数据集对应字段
        this.getFieldsByDataset(dataset.jumpId).then(rep => {
            //清楚数据分组数据
    ...
    render() {
        ...
        const _this = this;
        let { selectedName, auth, changeKeyword } = props;
        ...
        visibleList = list.filter(item => {
            return item.name.indexOf(lowerKeyword) > -1
        });
        if (visibleList.length) {
            visibleList = visibleList.sort((a, b) =>
                a.name.localeCompare(b.name)
            );
            numArr = visibleList.filter(e => /^\d/.test(e.name));
            letterArr = visibleList.filter(e => /^[a-zA-Z]/.test(e.name));
            chinaArr = visibleList.filter(e => /^[\u4e00-\u9fa5]/.test(e.name));
            specialArr = visibleList.filter(
                e => !/^(\d|[a-zA-Z]|[\u4e00-\u9fa5])/.test(e.name)
            );
