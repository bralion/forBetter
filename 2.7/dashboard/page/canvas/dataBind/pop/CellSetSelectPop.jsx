...
class CellSetSelectPop extends Component {
    ...
    selectEle() {
        ...
        if (_element) {
            ...
            //获取当前的cell
            let { valueKey, element } = this.props;
            let thisCell = element.find(e => e.valueKey === valueKey);
            collectTypeval == 0 &&
                getFieldType(type) !== "number" &&
                (selectArr = selectArr.filter(e => e[0].value !== "dataGroup"));
            thisCell.allowFieldTypes &&
                thisCell.allowFieldTypes.includes("number") &&
                (selectArr = selectArr.filter(e => e[0].value !== "dataGroup"));
        }
        return selectArr;
    }
    ...
    ifShowdidi(item, e) {
        ...
        if (valSp.length > 1) {
            ...
            //如果是数字类型的字段 选择无的时候 如果未进行查询 则添加查询 且更新字段
            valSp[0] === "collect" &&
                valSp[1] === "NONE" &&
                this.setNoneNum(valSp[0], valSp[1]);
        } else {
    ...
    //如果是数字类型的字段 选择无的时候 如果未进行查询 则添加查询 且更新字段
    setNoneNum() {
        let { config, field, group } = this.props.item;
        let { dataset, modelId, updateComponent } = this.props;
        let type = getFieldType(field.type);
        if (type !== "number") {
            return false;
        }
        let typeId = field.id + "__" + group.id;
        window.dataSetGroupObj = window.dataSetGroupObj || {};
        window.dataSetGroupObj[typeId] ||
            (window.dataSetGroupObj[typeId] = databindM.getFieldValues(
                field,
                dataset,
                modelId,
                true
