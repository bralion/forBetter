...
/**
 * 日期类型字段配置条件输入由日期选择器修改成了输入框。
 * 此方法根据字段信息获取placeholder，此placeholder对应为输入格式。
 * @param {*} field 
 */
function getPlaceholderByField(field){
    let { type, formulaStr } = field;
    ...
    switch(type){
        case 'date':{
            if(/^DATE_YEAR/.test(formulaStr)){
                return '格式 yyyy';
            }else if(/^DATE_QUARTER/.test(formulaStr)){
                return '格式 yyyy/Qn';
            }else if(/^DATE_MONTH/.test(formulaStr)){
                return '格式 yyyy/mm';
            }else if(/^DATE_WEEK/.test(formulaStr)){
                return '格式 yyyy/Wn';
            }else if(/^DATE_DAY/.test(formulaStr)){
                return '格式 yyyy/mm/dd';
            }else if(/^DATE_HOUR/.test(formulaStr)){
                return '格式 hh:00';
            }else if(/^DATE_MINUTE/.test(formulaStr)){
                return '格式 hh:MM';
            }else if(/^DATE_SECOND/.test(formulaStr)){
                return '格式 hh:MM:ss';
            }else{
...
const Range = props => {
    let { field,fieldType, value, onChange } = props;
...
class FilterConfigPop extends Component {
    ...
    valid(config) {
        ...
        if (config.filterType === 1) {
            ...
            let noNeedInputValues = [];// 记录不需要输入内容的操作符
                    ...
                    >
                        ...
                        <div style={{ overflow: "auto", maxHeight: "301px" }}>
                            {condition.children.map((item, idx) => {
                                ...
                                let placeholder;
                                if(item.operator && noNeedInputValues.indexOf(item.operator) === -1){
                                    placeholder = getPlaceholderByField(props.options.field);
                                }else{
                                    placeholder = '';
                                }
                                return (
                                    ...
                                    >
                                        ...
                                        <Col span={11}>
                                            ...
                                            ["GREATER_OR_EQUAL","LESS_OR_EQUAL"].indexOf(item.operator) > -1 ? (
                                                (()=>{
                                                    return  (
                                                        <Input 
                                                        ...
                                                        allowClear={true} 
                                                        onChange={e=>{
                                                            this.editCond(idx,"value",e.target.value);
                                                        }}/>
                                                    )
                                                })()
                                            ) : (
                                                <InputCom
                                                    ...
                                                    disabled={noNeedInputValues.indexOf(item.operator)> -1}
                                                    onBlur={e=>{
                                                        if(fieldType === 'number'){
                                                            let _v = Number(e.target.value);
                                                            if(isNaN(_v)){
