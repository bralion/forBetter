                ...
                <TreeNode
                    ...
                    title={
                        ...
                        >
                            ...
                            {item.calc && item.__group.id === "calc_field" ? <span className='databind-fields-item__name'>{item.name}</span> : item.name}
...
class Fields extends Component {
    ...
    bindDrag() {
        ...
        $(".databind-fields-item__field").draggable({
            ...
            start(event, ui) {
                ...
                el.find('.databind-fields-item__btns').hide();
                el.find('.databind-fields-item__name').css({top: 3,left: 27});
                el.find('.databind-icon-date').css({top: 5});
                dragObj = {
    ...
    render() {
        ...
        fields.forEach(item => {
            let columns = [];
            if (item.columns && item.columns.length) {
                columns = item.columns.filter(column => {
                    return column.name.indexOf(lowerKeyword) > -1
                });
            }
            if (columns.length) {
