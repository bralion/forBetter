...
import { dispatch } from "dashboard_2_7/dashboard/xdux";
...
class CanvasHandle extends Component {
    ...
    promiseComponent(type) {
        ...
        if (window.Dashboard.reactComponent[type]) {
            ...
            return import("dashboard_2_7/dashboard/components/" + type + "/" + type).then(
                e => {
    ...
    appendComponent(cfg){
        let type = cfg.type;
        this.promiseComponent(type).then(type => {
    ...
    setDroper(type, isIncontainer, serialNum) {
        ...
        this.setState(
            ...
            () => {
                ...
                isAutoPop = _components.map(e => e.type).findIndex(e => e === type) !== -1;
    ...
    openDataBindPop(widgetId) {
        ...
        let ins = Dashboard.compManager.getComponent(widgetId);
        let cfg = ins.cfg;
        cfg.chartDefinition.query.isNoQuery = true;
        let _cfg = util.deepClone(cfg);
        //存储下转数据
        ins.drillData && (_cfg.drillData = { ...ins.drillData });
