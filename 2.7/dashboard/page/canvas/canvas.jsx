...
import $ from "jquery";
import "dashboard_2_7/dashboard/util/react-grid-layout/layout.css";
import "dashboard_2_7/dashboard/util/react-grid-layout/resizable.css";
import dot from "dashboard_2_7/dashboard/resource/images/dot.png";
import Util from "dashboard_2_7/dashboard/modules/common/Util";
import ReactGridLayout from "dashboard_2_7/dashboard/util/react-grid-layout/index";
import widgetComponent from "dashboard_2_7/dashboard/components/widgetComponent";
...
const util = new Util();
let DataBindPop = asyncComponent(() => import("dashboard_2_7/dashboard/page/canvas/dataBind"));
...
class Canvas extends canvasHandle {
    ...
    componentDidMount() {
        ...
        // 订阅事件，支持追加的方式向画布组件
        Dashboard.event.subscribe('CANVAS_APPEND_COMPONENT',(cfg)=>{
            this.appendComponent(cfg);
        });

        if(!dashboardMode.path){
            window.Dashboard.noticeTopWindow(null,'componentRendered', { Dashboard } );
        }
    }
    componentWillUnmount(){
        Dashboard.event.unsubscribe('CANVAS_APPEND_COMPONENT');
    }
    onLayoutChange() {
    ...
    isShowBg() {
        let { state, editeMode } = this.props;
        let { components, isReEditReq } = this.state;
        let isShowBg = false;
        if (!editeMode) {
            if (state == 2) {
                if (isReEditReq) {
                    isShowBg = !components.length;
                }
            } else {
                isShowBg = !components.length;
            }
