...
import { dispatch, xuduListener } from "dashboard_2_7/dashboard/xdux/index";
import {
    Input,
    Select,
    Row,
...
class Sqltab extends Component {
    ...
    fetchData() {
        service({
            method: "get",
            url: "/xdatainsight/api/workspace/data/connection?workspaceId=" + window.dashboardMode.workspaceId,
            responseType: "json"
        }).then(response => {
    ...
    handleData(data) {
        ...
        // let _select = this.state.select || options[0] || "";
        let options = data || [];
        let _select;
        if(this.state.select){
            _select = this.state.select
        }else if(options.length>0){
            _select = options[0].fileGuid
        }else{
            _select = ''
        }
        this.setState(
                    ...
                    <Col span={18}>
                        ...
                        >
                            {this.state.sqlOptions.map((el, i) => (
                                <Option value={el.fileGuid} key={i}>
                                    {el.fileName}
                                </Option>
                            ))}
                        </Select>
