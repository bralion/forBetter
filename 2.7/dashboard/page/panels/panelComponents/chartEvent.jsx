...
function copy(props) {
    ...
    return _widget.option.filter(e =>
        props.noDrill ? e.value !== "drill" : true
    );
}
class Dirll extends Component {
    ...
    widgetChange(type, e) {
        let _option = copy(this.props);
        let { option, query } = this.inst.cfg.chartDefinition;
        if (
            _option.find(e => e.value === "drill") &&
            query.type === "dataset"
        ) {
            this.inst.cfg.chartDefinition.option.clickEvent = e;
            this.inst.doQuery();
        } else {
